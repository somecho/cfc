...
void updateParticles(Particles *p) {
  ...
  for (size_t i = 0; i < p->numParticles; i++) {
    if (p->age[i] < p->maxAge[i]) {
      ...
      p->pos[i][0] = ((float)rand() / (float)RAND_MAX) * ccGetWidth();
      p->pos[i][1] = ((float)rand() / (float)RAND_MAX) * ccGetWidth();
      p->heading[i] = ((float)rand() / (float)RAND_MAX) * M_PI * 2;
      p->speed[i] = ((float)rand() / (float)RAND_MAX) * 2.0 + 1.0;
      p->age[i] = 0;
      p->maxAge[i] = rand() % 200;
    }
  }
...
void setup() {
  srand(time(NULL));
  ccSetDrawMode(GL_POINTS);
  // Initialize the particle system
  ps.numParticles = NUM_PARTICLES;
  for (size_t i = 0; i < NUM_PARTICLES; i++) {
    ps.pos[i][0] = ((float)rand() / (float)RAND_MAX) * ccGetWidth();
    ps.pos[i][1] = ((float)rand() / (float)RAND_MAX) * ccGetWidth();
    ps.pos[i][2] = 0;
    ps.heading[i] = ((float)rand() / (float)RAND_MAX) * M_PI * 2;
    ps.speed[i] = ((float)rand() / (float)RAND_MAX) * 2.0 + 1.0;
    ps.age[i] = 0;
    ps.maxAge[i] = rand() % 500;
  }
}
